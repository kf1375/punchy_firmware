 ...
 *
 */
WebServer::WebServer(mg_mgr &mgr, Configuration &config,
...
void WebServer::setup()
{
  ...
  m_conn = mg_http_listen(
      ...
      [](mg_connection *c, int ev, void *ev_data) {
        static_cast<WebServer *>(c->fn_data)->eventHandler(c, ev, ev_data);
      },
      this);
}
 ...
 *
 * match API requests to the corresponding functions and send filesystem static
 * files if no api url matches
 */
void WebServer::eventHandler(struct mg_connection *c, int ev, void *ev_data)
{
  if (ev == MG_EV_HTTP_MSG) {
    ...
    if (mg_match(hm->uri, mg_str("/api/wifi/set"), NULL)) {
      ...
      mg_http_serve_file(c, hm, "/littlefs/static/captive_page.html",
                         &m_httpServeOpts);
...
void WebServer::handleSetWifi(struct mg_connection *c,
...
void WebServer::handleGetWifi(struct mg_connection *c,
                              struct mg_http_message *hm)
{
  JsonDocument doc;
  typedef enum {
  ...
  if (status == WL_CONNECTED) {
    ...
    mg_timer_add(
        &m_mgr, 2000, 0,
        [](void *arg) { static_cast<WebServer *>(arg)->stop(); },
        this); // stop the server after 5 seconds
...
void WebServer::sendTextResponse(struct mg_connection *c, size_t code,
                                 String text)
{
...
String WebServer::extractStringVariable(struct mg_http_message *hm,
                                        String variable)
{
...
float WebServer::extractFloatVariable(struct mg_http_message *hm,
                                      String variable)
{
